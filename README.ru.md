...
- [Экспериментально: кастомный IP-протокол](#экспериментально-кастомный-ip-протокол)
...
```bash
./pingtunnel --type client -l :4455 -s СЕРВЕР -t СЕРВЕР:4455 --udp_rel 1 --key 123456
```
...
## Экспериментально: кастомный IP-протокол

> ⚠️ **Экспериментальный режим. Не является надёжным решением - см. ограничения ниже.**

По умолчанию транспорт работает поверх ICMP (IP-протокол 1). Флаг `--ip_proto N`
позволяет использовать тот же wire-формат поверх другого номера IP-протокола -
например `253` или `254`, зарезервированных IANA под эксперименты (RFC 3692).
Формат пакета не изменяется (те же 8 байт псевдо-echo заголовка и protobuf);
изменяется только поле `protocol` в IP-заголовке, формируемое ядром.

Назначение: в ряде сетей пропускная способность ICMP ограничивается на уровне
класса трафика (DPI или шейпер, нацеленный на ICMP). Использование другого
IP-протокола позволяет не попадать под такое ограничение, специфичное для ICMP.
При измерении достигнута скорость порядка 150-160 Мбит/с с `--jumbo 1400` на
канале, где пропускная способность ICMP была значительно ограничена.
...
- Маскировка под ICMP-эхо отсутствует.
...
# клиент
./pingtunnel --type client -l :1080 -s СЕРВЕР --sock5 1 --key 123456 --ip_proto 253
```
...
трафик по множеству IP-протоколов, сам по себе - заметная аномалия для DPI:
ротация помогает против пер-флоу шейпинга, но не против обнаружения.
...
# клиент
./pingtunnel --type client -l :1080 -s СЕРВЕР --sock5 1 --key 123456 --ip_proto_range 100-254
```
...
| `--ip_proto`    | номер IP-протокола транспорта (1 = ICMP; другой = **эксперимент**, напр. 253) | `1`           |
