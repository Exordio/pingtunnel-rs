...
**Главный «хак» под throughput — крупнее кадр** (меньше пакетов на байт →
меньше pps → ниже CPU). В этой ветке кадр фиксированный (888 Б, маскировка под
обычный ping); экспериментальная async-ветка `async-tokio` добавляет флаг
`--jumbo` и батчинг syscalls (`recvmmsg`/`sendmmsg`) — см. ниже.

### Ветка `async-tokio` (эксперимент)

Параллельно есть перепись на tokio (event-driven, без потока-на-соединение,
батчинг syscalls, скромные буферы). Её профиль: **память ~20 МБ против ~520 МБ**
у этой версии, но throughput ниже (~2.5–2.7 МБ/с) и CPU под нагрузкой сравним
(ICMP всё равно pps-связан — батчинг почти не включается при умеренном pps).
Берите её, если критична **память**; эту (поточную) — если важнее **throughput
и стабильность**.
