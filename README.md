...
Pingtunnel — инструмент, который **туннелирует TCP/SOCKS5-трафик поверх ICMP**
...
> настраиваемый размер кадра (`--jumbo`). Поддержаны TCP и SOCKS5-CONNECT; UDP —
> в поточной версии в истории git (см. «Архитектура»).
...
| `src/socks5.rs`      | SOCKS5: async-рукопожатие сервера, разбор запросов, кодирование адресов |
| `src/forward.rs`     | async-форвардинг через socks5/http-прокси                            |
| `src/client.rs`      | клиент: приём локальных TCP/SOCKS5, туннелирование в ICMP             |
...
(`sendmmsg`). `FrameMgr` каждого соединения — единоличный владелец (без `Mutex`).
Профиль: память ~20–150 МБ (зависит от числа соединений), скромные буферы.

> В этой async-версии поддержаны **TCP и SOCKS5 (CONNECT)**. UDP и SOCKS5 UDP
> ASSOCIATE — пока нет (есть в поточной версии в истории git, тег/ветка до перехода
> на tokio). Если нужен UDP — берите её.
